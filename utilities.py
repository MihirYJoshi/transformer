...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            am = am.squeeze(0).detach().cpu().numpy()  # Remove batch dimension and convert to NumPy array
            for i in range(0,2):
                newMap = torch.tensor(am[i])
                # Check if the attention probabilities sum to 1 over rows
                total_prob_over_rows = torch.sum(newMap, dim=0)
                ...
                fig, ax = plt.subplots()
                cax = ax.imshow(am[i], cmap='hot', interpolation='nearest')
                ax.xaxis.tick_top()  
                fig.colorbar(cax, ax=ax)  
                plt.title(f"Attention Map {j + 1} head {i}")
                
                # Save the plot
                plt.savefig(f"attention_map_{j + 1}_head_{i}.png")
